...
describe('mutations', () => {
  describe('adding a mutation', () => {
    it('should add mutation type rule to end of rules array', () => {
      expect(allReplaceText[4].clauses[0].rules.length).toBe(2)
    })
    it('should add default mutation to end of rules array', () => {
      expect(allReplaceText[2].clauses[0].rules.slice(-1).pop()).toMatchObject(defaultMutation)
    })
  })
  describe('activating a mutation', () => {
    it('should set as active', () => {
      expect(allReplaceText[5].clauses[0].rules.slice(-1).pop().active).toBeTruthy()
    })
  })
})
...
describe('mutation denormalizer', () => {
  it('should mutate value as a result of mutation reduction', () => {
    ...
    let result = mutationDenormalizer(story.clauses[0].view, story.slave.list.open, story.clauses[0].rules)
    ...
    result = mutationDenormalizer(story.clauses[0].view, story.slave.list.open, story.clauses[0].rules)
    result.forEach(({id, value}) => {
